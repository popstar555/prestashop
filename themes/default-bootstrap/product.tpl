...
<div itemscope itemtype="http://schema.org/Product">
	<div class="primary_block row">
		...
		<div class="pb-center-column col-xs-12 col-sm-4">
			...
			{/if}
			{if $product->description_short || $product->description || $packItems|@count > 0}
				<div id="short_description_block">
					{if $product->description_short|@strlen > $product->description|@strlen}
			...
			<form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
				...
				<div class="box-info-product">
					...
					<div class="product_attributes clearfix">
						<!-- quantity wanted -->
						<div class="metainfo_producto_container" style="display: none;">
							<strong>{l s='Superficie'} (m<sup>2</sup>)</strong><br />
							<input type="text" id="metainfo_producto_metros" /><br />
							<div id="metainfo_producto_resto" style="display: none; font-style: italic;">
								Compra <span class="unidades"></span> unidad(es), sobran <span class="metros"></span> m<sup>2</sup>.
								<br /><br />
							</div>
							<strong>{l s='Peso'}</strong><br />
							<div id="metainfo_producto"></div><br />
						</div>
						{if !$PS_CATALOG_MODE}
					...
					var _superficie = null;
					var _peso = null;
						{if isset($features) && $features}
							{foreach from=$features item=feature}
								{if isset($feature.value)}
									{if $feature.id_feature == 8}
										var _superficie = {$feature.value};
									{/if}
									{if $feature.id_feature == 4}
										var _peso = {$feature.value};
									{/if}
								{/if}
							{/foreach}

							if(_superficie && _peso) {
								var newSurface = (1 * _superficie);
								var newWeight = (1 * _peso);
								var newSurfaceString =  newSurface;
								var newWeightString =  newWeight + " kg.";
								$('#metainfo_producto_metros').val(newSurfaceString);
								$('#metainfo_producto').text(newWeightString);
								$('.metainfo_producto_container').show();
							}
						{/if}
					</script>
